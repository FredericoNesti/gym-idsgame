"""
An agent for the IDSGameEnv that implements the REINFORCE with BAYESIAN Baseline (Critic) Policy Gradient algorithm.
"""
from typing import Union, List
import numpy as np
import numpy.linalg.inv as minv
import time
import tqdm
import torch
import copy
...
class BayesActorCriticAgent(PolicyGradientAgent):
    """
    ...
    def __init__(self, env:IdsGameEnv, config: PolicyGradientAgentConfig):
        ...
        """
        super(BayesActorCriticAgent, self).__init__(env, config)
    ...
    @staticmethod
    def H(gamma, V):
        H = np.zeros((V.shape[0], V.shape[1]+1))
    ...
    @staticmethod
    def Rprev(H, V, Sigma):
        N = random.multivariate_normal(mean = np.zero((Sigma.shape[0])), cov = Sigma)
        Rprev = H @ V + N
        return Rprev
    ...
    #@staticmethod
    def GPTD_step(self, value : List[torch.Tensor], log_probs : List[torch.Tensor], gamma=0.9, sigma=1.0):
        """
        value can stand for state_value (V) or action_value (Q), as the GPTD algorithm is similar

        This function is to compute the GPTD bayesian update for 1 episode of the gradient.
        The convergence criteria of BAC may be implemented outside of this function

        state_value input here is the prior

        :input: u(z), state_value or action_value
        :return: posterir value mean and covar
        """
        t = len(value)

        U = np.array(log_probs)
        Ghat = (1 / (t + 1)) * U @ U.T
        ...
        K = BayesActorCriticAgent.kernel(t, U, Ghat)
        Q = minv(H @ K @ H.T + Sigma)
        a = H.T @ Q @ Rprev
        C = H.T @ Q @ H

        return a, C, U, Ghat, Rprev

    '''
    def update_policy_grads(self, a, C, U, G, beta):
        """
        This function is to update in a Bayesian way the weights of the actor network
        so as to output, bayesian policy probs

        :input: state_values, the bayesian mean and covar of the state_value distribution
        :return: the policy updated gradients to substitute for the next train loop under epochs.
        """
        delta = minv(G) @ U @ a
        with torch.no_grad():
            self.attacker_policy_network.parameters().copy_(grads + beta*delta)
        return delta
    '''
    def training_step(self, saved_rewards : List[List[float]], saved_log_probs : List[List[torch.Tensor]],
        ...
        for batch in range(num_batches):
            ...
            for log_prob, state_value, R in zip(saved_log_probs[batch], saved_state_values[batch], returns):
                ...
                a, C, U, G, Rprev = self.GPTD_step(state_value, log_prob)
                ...
                '''
                ##########
                ###### From here we can run GPTD for Bayes updating of the next gradients for the actor-critic network
                ## the number of M episodes is already defined on the length of the inputs (log_prob and state_value)
                eps = 0.01
                done = False
                while not done:
                    a, C, U, G = self.GPTD_step(state_value, log_prob)
                    delta = self.update_policy_grads(a, C, U, G, beta=0.001)
                    if delta < eps: done = True
                ###########
                '''
